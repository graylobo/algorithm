...
// let result = 0;
// while (start <= end) {
//   let mid = parseInt((start + end) / 2); // 현재의 중간점 (절단기 높이 설정)
//   let total = 0;
//   for (x of arr) {
//     if (x > mid) total += x - mid; // mid(절단기 높이)로 잘랐을 때 얻을 수 있는 나무의 양 계산
//   }
...
let [n, 필요한나무길이양] = input[0].split(" ").map(Number);
let arr = input[1].split(" ").map(Number); // 각 나무의 높이
let start = 0;
let end = arr.reduce((a, b) => Math.max(a, b));
let result = 0;
while (start <= end) {
  let 구한나무길이 = 0;
  let 절단기높이 = parseInt((start + end) / 2);
  for (const 나무길이 of arr) {
    if (나무길이 >= 절단기높이) 구한나무길이 += 나무길이 - 절단기높이;
  }
